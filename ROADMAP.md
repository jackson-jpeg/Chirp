...
- [x] Network diagnostics view — long-press mesh radar to show peers in range, packets relayed, dedup rate, hops observed, WiFiAware link metrics. Files: `Chirp/Sources/ViewModels/AppState.swift`, new `Chirp/Sources/Views/DiagnosticsView.swift`
- [x] Message delivery status icons — extend DeliveryStatus to include `.read`, send READ! ACK when message scrolled into view. Show checkmark progression (sent/delivered/read). Files: `Chirp/Sources/Views/Components/MessageBubbleView.swift`, `Chirp/Sources/Services/TextMessageService.swift`
- [x] Lazy message pagination — load last 50 messages on appear, load older batches on scroll-to-top. Cap memory cache at 200 messages. Files: `Chirp/Sources/Services/Persistence/LighthouseDatabase.swift`, `Chirp/Sources/Services/TextMessageService.swift`
